...
namespace Consolediff.filetree
{
    class ДеревоМетаданных
    {
        ...
        public List<ДеревоМетаданных> ПолучитьРазличияРекурсией(ДеревоМетаданных ВетвьДерева2)
        {
            ...
            foreach (var Лист in Подчиненные)
            {
                ...
                if(Лист2!=null)
                {
                    var Различия = Лист.ПолучитьРазличияРекурсией(Лист2);
                   // if (Различия.Count > 0) Лист.СуммаВложенныхРазличий = 1;// Различия.Count;
