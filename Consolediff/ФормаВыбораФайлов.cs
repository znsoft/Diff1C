    ...
    public partial class ФормаВыбораФайлов : Form
    {
        ...
        private void КнопкаНачатьСравнение_Click(object sender, EventArgs e)
        {
            ...
            ДеревоМетаданных Дерево2 = new Загрузчик(ИмяФайла2.Text, 3, (В, с, р) => { Прогресс.Maximum = (int)р; Прогресс.Increment(с.Length); }).КореньДерева;
             var РазличияСписком = Дерево1.ПолучитьРазличияРекурсией(Дерево2, (В, с, р) => {  Прогресс.Increment(1); });
            РазличияСписком.AddRange(Дерево2.ПолучитьРазличияРекурсией(Дерево1, (В, с, р) => {  Прогресс.Increment(1); }));
            
            СписокИзменений.Items.Clear();
             РазличияСписком = (from Различие in РазличияСписком where Различие.уровень ==2 &&  Различие.СуммаВложенныхРазличий > 0 select Различие).ToList();
             foreach (var Различие in РазличияСписком) {
                ...
                Прогресс.Increment(1);
            }
            ДеревоИзменений.Nodes.Clear();
            ДеревоИзменений2.Nodes.Clear();
